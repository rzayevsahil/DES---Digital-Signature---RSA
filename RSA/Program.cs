...
namespace RSA
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int d = D_Bulma(phi, e);
            //Console.WriteLine("d:{0} e:{1} n:{2}", d, e, n);
            Console.ForegroundColor = ConsoleColor.DarkYellow;
            Console.WriteLine("\n\nGenel anahtar(n:{0},e:{1})", n, e);
            Console.WriteLine("\nÖzel anahtar(d:{0})", d);
            ...
            string sifrelenmisMetin = Encrypt(text, e, n);


            string cozulmusMetin = Decrypt(sifrelenmisMetin, d, n);
            ...
            string fromAsciiToText = null;
            for (int i = 0; i < asciiDonusum.Length; i++)
            {
                fromAsciiToText += Char.ConvertFromUtf32(Convert.ToInt32(asciiDonusum[i]));
            }
        ...
        static string Encrypt(string text, int e, int n)
        {
            int L;
            L = BasamakSayisi(n);
            int L_clear = L - 1;
            ...
            {
                Console.Write(m[i] + "\t");
                tekTeksifrelenmisDegerler[i] = UstelMod(m[i], e, n);
            }

            Console.WriteLine("\n\n\nmod alınmış değerler - n:{0} e:{1}\n", n, e);
        ...
        static string Decrypt(string text, int d, int n)
        {
            ...
            for (int i = 0; i < ikiBasamakliTekTekDesifrelenmisDegerler.Length; i++)
            {
                if (ikiBasamakliTekTekDesifrelenmisDegerler[ikiBasamakliTekTekDesifrelenmisDegerler.Length - 1] == "0")
                {
                    continue;
                }
                else
            ...
            if (cozulmusMetin[cozulmusMetin.Length - 1] == '0')
            {
                cozulmusMetin.Remove(cozulmusMetin.Length - 1, 1);
            }
            ...
            for (int i = 0; i < ucluDesifrelenmisAsciiDegerler.Length; i++)
            {
                ...
                cozulmusMetin += ucluDesifrelenmisAsciiDegerler[i];
            }
